...
frames = range(0, 670)
axis1 = cmds.ls(sl=1)[0]
axis2 = cmds.ls(sl=1)[1]
path = "testdaten/achsen2/kura/rot-{}-{}.csv".format(axis1.replace(":", "_"), axis2.replace(":", "_"))
path = os.path.abspath(path)
o_file = open(path, 'w')
o_file.write("frame,RXY,RXZ,RYZ,X1,Y1,Z1,X2,Y2,Z2\n")
...
for frame in frames:
    ...
    o_file.write('{},"{}","{}","{}","{}","{}","{}","{}","{}","{}"\n'.format(frame, angle_xy_plane, angle_xz_plane, angle_yz_plane,
        transl1[0], transl1[1], transl1[2], transl2[0], transl2[1], transl2[2]))
    #o_file.flush()
o_file.close()