...
ground_breaks_group = c(c("beton", "kura"),
                        c("beton", "karera"),
                        c("beton", "pedikura"))
...
{
  joint_grp = joint_grps[[joint_grp_name]]
  g <- ggplot(interpolations_means[joint_name%in%joint_grp & ground_type%in%ground_breaks, ], 
    ...
    theme_bw()+
    theme(legend.justification=c(1,0))
  ggsave(g, file=paste(c("plot_", "average","_", joint_grp_name , ".pdf"), collapse=""), width=8, height=10, device=cairo_pdf)
}
...
for(joint_grp_name in names(joint_grps))
{
  ...
  g <- ggplot(interpolations_means[joint_name%in%joint_grp & ground_type%in%ground_breaks, ], 
    ...
    theme_bw()+
    theme(legend.justification=c(1,0))
  ggsave(g, file=paste(c("plot_", "average","_", joint_grp_name,"_without_ribbons" , ".pdf"), collapse=""), width=8, height=10, device=cairo_pdf)
}
...
for(joint_grp_name in names(joint_grps))
{
  ...
  g <- ggplot(interpolations_means[joint_name%in%joint_grp & ground_type%in%ground_breaks, ], 
    ...
    theme_bw()+
    theme(legend.justification=c(1,0))
  ggsave(g, file=paste(c("plot_", "average","_", joint_grp_name,"_without_error_indicators" , ".pdf"), collapse=""), width=8, height=10, device=cairo_pdf)
}
