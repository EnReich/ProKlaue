# This script reads in angle data.
...
setwd("~/ProKlaue - Sabrina/ergebnisse2")
...
replaceUmlauts <- function(str){stri_replace_all_fixed(str, 
                                                c("ä", "Ä", "ö", "Ö", "ü", "Ü", "ß"),
                                                c("ae", "Ae", "oe", "Oe", "ue", "Ue", "ss"),
                                                vectorize_all = FALSE)}
...
animal_breaks = c("Fanta","Rapunzel", "Paperlapapp")
animal_labels = c("Fanta" = "1", "Rapunzel" = "2", "Paperlapapp" = "3")
...
#                                      "Fesselträger", "Seitenkeil", "Standard", 
#                                      "Trachtenkeil", "Zehenoffen"), 
#                                    c("fest", "weich"), 
#                                    sep = "_"))
# names(ground_labels) = ground_labels

ground_labels_names = levels(interaction(c("Barhuf","Breitschenkel","Eiereisen", 
                                     "Fesselträger", "Seitenkeil", "Standard", 
                                     "Trachtenkeil", "Zehenoffen"), 
                                   c("fest", "weich"), 
                                   sep = "_"))
ground_labels = levels(interaction(c("Barhuf","Breitschenkel","Eiereisen", 
                                     "Eisen mit verbreiterter Zehe", "Seitenkeil", "Standard", 
                                     "Trachtenkeil", "Zehenoffen"), 
                                   c("(fest)", "(weich)"), 
                                   sep = " "))
names(ground_labels) = ground_labels_names
...
labels4 = c("R1_fitted_inv"= paste(c("\u2190 Extension     ", "Z",  "       Flexion \u2192"), collapse=collapse_sequence), 
            ...
            "RF_fitted"=     paste(c("\u2190 Innenrotation",  "Y",  "Außenrotation \u2192"), collapse=collapse_sequence))
labels4_raw <- labels4
...
ground_grps = c(ground_grps, list(all = ground_breaks,
                                  ...
                                  Fesselträger = c("Fesselträger_weich")))
...
ground_grps_single = list(c("Barhuf_fest", "Barhuf_weich"),
                          ...
                          c("Standard_weich", "Fesselträger_weich"),
...
#single ground grps to plot
ground_grps_single2 = list(c("Barhuf_fest", "Barhuf_weich"),
                          c("Barhuf_fest", "Standard_fest"),
                          c("Barhuf_weich", "Standard_weich"),
                          c("Standard_fest", "Standard_weich"),
                          c("Standard_weich", "Fesselträger_weich"),
                          c("Standard_fest", "Eiereisen_fest"),
                          c("Standard_weich", "Eiereisen_weich"),
                          c("Standard_fest", "Zehenoffen_fest"),
                          c("Standard_weich", "Zehenoffen_weich"),
                          c("Standard_fest", "Seitenkeil_fest"),
                          c("Standard_fest", "Trachtenkeil_fest"),
                          c("Standard_weich", "Breitschenkel_weich")
                          )

names(ground_grps_single2) = lapply(ground_grps_single2, paste, collapse = "_vs_")
...
qualitative_color_palette_ground_types["Barhuf_fest"] = "#e66101"
qualitative_color_palette_ground_types["Barhuf_weich"] = "#fdb863"
qualitative_color_palette_ground_types["Standard_fest"] = "#5e3c99"
qualitative_color_palette_ground_types["Standard_weich"] = "#b2abd2"

qualitative_color_palette_ground_types[which(!names(qualitative_color_palette_ground_types)%in% 
        c("Barhuf_fest", "Barhuf_weich", "Standard_fest", "Standard_weich"))] = 
  brewer.pal(n= length(qualitative_color_palette_ground_types)-3, name = "YlGn")[-c(1)]


qualitative_color_palette_animal_types <- brewer.pal(n=length(animal_breaks), name="Dark2")
...
contact_times_table_imp = data.table(read.csv("../Anfangs_und_Endframes_2.csv"))
...
#------- SCALES ---------------
minandmax= data[(variable %in% plot_vars_4) & 
                  ground_type%in%ground_breaks&
                  time>time_plot_min&
                  time<time_plot_max, list(min=min(value), max =max(value)), by=list(joint_name, variable) ]

setkeyv(minandmax, c("joint_name", "variable"))

scales_rotations_uniform <- list(
  
  "DIP"= list(
    "R2_fitted" = scale_y_continuous(limits = c(-12,5), oob = rescale_none, name = "Rotation (°)"),
    "RF_fitted" = scale_y_continuous(limits = c(-10,16), oob = rescale_none, name = "Rotation (°)"),
    "R1_fitted_inv" = scale_y_continuous(limits = c(-21,37), oob = rescale_none, name = "Rotation (°)")
    ),
  
  "PIP"= list(
    "R2_fitted" = scale_y_continuous(limits = c(-7,12), oob = rescale_none, name = "Rotation (°)"),
    "RF_fitted" = scale_y_continuous(limits = c(-12,13), oob = rescale_none, name = "Rotation (°)"),
    "R1_fitted_inv" = scale_y_continuous(limits = c(-6,16), oob = rescale_none, name = "Rotation (°)")
  )
)
...
# --------------- PLOTS -------------------------------------
plot_dir_single_animal = file.path(plots_dir, "single_animal")
if(!dir.exists(plot_dir_single_animal))
{
  dir.create(path = plot_dir_single_animal, recursive = T)
}

plot_dir_single_animal_uniform = file.path(plots_dir, "single_animal_uniform")
if(!dir.exists(plot_dir_single_animal_uniform))
{
  dir.create(path = plot_dir_single_animal_uniform, recursive = T)
}

plot_dir_single_ground = file.path(plots_dir, "single_ground")
if(!dir.exists(plot_dir_single_ground))
{
  dir.create(path = plot_dir_single_ground, recursive = T)
}

plot_dir_single_ground_uniform = file.path(plots_dir, "single_ground_uniform")
if(!dir.exists(plot_dir_single_ground_uniform))
{
  dir.create(path = plot_dir_single_ground_uniform, recursive = T)
}

single_grps2_dir = file.path(plots_dir, "grps")
if(!dir.exists(single_grps2_dir))
{
  dir.create(path = single_grps2_dir, recursive = T)
}

single_grps2_dir_uniform = file.path(plots_dir, "grps_uniform")
if(!dir.exists(single_grps2_dir_uniform))
{
  dir.create(path = single_grps2_dir_uniform, recursive = T)
}
...
for(animal in levels(data$animal_name))
{
  for(joint_grp_name in names(joint_grps))
  {
    ...
    base <- ggplot(data[(variable %in% plot_vars_4) & 
                          animal_name==animal & 
      ...
      geom_line(data=data_replaced[(variable %in% plot_vars_4) & 
                                     animal_name==animal & 
                                     joint_name%in%joint_grp & 
                                     ground_type%in%ground_breaks&
                                     time>time_plot_min&
    ...
    g <-base + 
      facet_grid(list("variable", c("joint_name")), 
                 scales = "free_y", 
                 labeller=labeller(variable = labels4))
    
    g_uniform <- base + 
      facet_grid_sc(rows = vars(variable), cols = vars(joint_name), 
                    scales = list(y=scales_rotations_uniform[[joint_grp]]), 
                    labeller=labeller(variable = labels4))
  

    ggsave(g, file=replaceUmlauts(file.path(plot_dir_single_animal, 
                                            paste(c("plot_", animal,"_", joint_grp_name , ".pdf"), collapse=""))), 
           width=8, height=10, device=cairo_pdf)
    ggsave(g_uniform, file=replaceUmlauts(file.path(plot_dir_single_animal_uniform, 
                                            paste(c("plot_", animal,"_", joint_grp_name , "_uniform.pdf"), collapse=""))), 
           width=8, height=10, device=cairo_pdf)
  }
}
...
for(ground in levels(data$ground_type))
{
  for(joint_grp_name in names(joint_grps))
  {
    ...
    joint_label_c = c(paste(joint_grp, ", ", ground_labels[ground], sep= ""))
    names(joint_label_c) = c(joint_grp)
    ...
    base <- ggplot(data[(variable %in% plot_vars_4) &
      ...
      geom_line(data=data_replaced[(variable %in% plot_vars_4) &
                                     ...
                                     time>time_plot_min&
                                     time<time_plot_max, ])+
      scale_color_manual(name = cat_animals_lab, breaks = animal_breaks, labels=animal_labels, values = qualitative_color_palette)+
      scale_fill_manual(name = cat_animals_lab, breaks = animal_breaks, labels=animal_labels, values = qualitative_color_palette)+
      scale_x_continuous(name = "Zeit (%)", limits = c(0,100), breaks=c(0,20,40,60,80,100))+
      scale_y_continuous(name = "Rotation (°)")+
      theme_bw()+
      # theme(legend.justification = c(1,0))
    ...
    ggsave(g, file=replaceUmlauts(file.path(plot_dir_single_ground, 
                                            paste(c("plot_", ground,"_", joint_grp_name , ".pdf"), collapse=""))), 
           width=8, height=10, device=cairo_pdf)
    
    ggsave(g_uniform, file=replaceUmlauts(file.path(plot_dir_single_ground_uniform, 
                                            paste(c("plot_", ground,"_", joint_grp_name , "_uniform.pdf"), collapse=""))), 
           width=8, height=10, device=cairo_pdf)
  }
}
...
for(animal in levels(data$animal_name))
{
  for(joint_grp_name in names(joint_grps))
  {
    ...
    joint_label_c = c(paste(joint_grp, ", Pony-Nr. ", animal_labels[animal], sep= ""))
    names(joint_label_c) = c(joint_grp)
    
    for(ground_grp_idx in seq(ground_grps_single2))
    {
      ground_grp = ground_grps_single2[[ground_grp_idx]]
      ground_grp_name = names(ground_grps_single2)[ground_grp_idx]
      ...
      base <- ggplot(data[(variable %in% plot_vars_4) & 
        ...
        geom_line(aes(y=value_max_error), linetype=2, alpha =0.7)+
        geom_line(
          # data=data_replaced[(variable %in% plot_vars_4) & 
          #                              animal_name==animal & 
          #                              joint_name%in%joint_grp & 
          #                              ground_type%in%ground_grp&
          #                              time>time_plot_min&
        ...
        # scale_color_manual(name="Bodentyp", breaks = ground_breaks, labels=ground_labels, values = qualitative_color_palette)+
        # scale_fill_manual(name = "Bodentyp", breaks = ground_breaks, labels=ground_labels, values = qualitative_color_palette)+
        scale_color_manual(name="Bodentyp", breaks = ground_breaks, labels=ground_labels, values = brewer.pal(n = 8, name = "Dark2"))+
        scale_fill_manual(name = "Bodentyp", breaks = ground_breaks, labels=ground_labels, values = brewer.pal(n = 8, name = "Dark2"))+
        scale_x_continuous(name = "Zeit (%)", limits = c(0,100), breaks=c(0,20,40,60,80,100))+
        scale_y_continuous(name = "Rotation (°)")+
        theme_bw()+
        theme(
      ...
      ggsave(g, file=replaceUmlauts(file.path(single_grps2_dir,paste(c("plot_",ground_grp_name, "_", animal,"_", joint_grp_name ,".pdf"), collapse=""))), 
             width=8, height=10, device=cairo_pdf)
      ggsave(g_uniform, file=replaceUmlauts(file.path(single_grps2_dir_uniform,paste(c("plot_",ground_grp_name, "_", animal,"_", joint_grp_name ,"_uniform.pdf"), collapse=""))), 
             width=8, height=10, device=cairo_pdf)
      ...
      #   # theme(legend.justification = c(1,0))
      #   theme(legend.position="bottom")
      # ggsave(g, file=replaceUmlauts(paste(c(file.path(single_grps2_dir), "/", "plot_",ground_grp_name, "_", animal,"_", joint_grp_name ,"_wo_err_indic.pdf"), collapse="")), width=8, height=10, device=cairo_pdf)
      # 
    }
  }
}
...
#       # theme(legend.justification=c(1,0))
#       theme(legend.position="bottom")
#     ggsave(g, file=replaceUmlauts(paste(c(plots_dir, "/average_grps/", "plot_", "average","_",ground_grp_name, "_", joint_grp_name , ".pdf"), collapse="")), width=8, height=10, device=cairo_pdf)
#   }
#   
#   # 
#   # for(joint_grp_name in names(joint_grps))
...
#       # theme(legend.justification=c(1,0))
#       theme(legend.position="bottom")
#     ggsave(g, file=replaceUmlauts(paste(c(plots_dir, "/average_grps/", "plot_", "average","_", ground_grp_name, "_", joint_grp_name,"_wo_err_indic" , ".pdf"), collapse="")), width=8, height=10, device=cairo_pdf)
#   }
# }
...
#         theme_bw()+
#         theme(legend.justification = c(1,0))
#       ggsave(g, file=replaceUmlauts(paste(c(plots_dir, "/single_grps/", "plot_",ground_grp_name, "_", animal,"_", joint_grp_name ,".pdf"), collapse="")), width=8, height=10, device=cairo_pdf)
#       
#       g <- ggplot(data[(variable %in% plot_vars_4) & 
#                          animal_name==animal & 
#                          joint_name%in%joint_grp & 
...
#         theme_bw()+
#         theme(legend.justification = c(1,0))
#       ggsave(g, file=replaceUmlauts(paste(c(plots_dir, "/single_grps/", "plot_",ground_grp_name, "_", animal,"_", joint_grp_name ,"_wo_err_indic.pdf"), collapse="")), width=8, height=10, device=cairo_pdf)
#       
#     }
#   }
# }
